...
class special_layer(keras.layers.Layer):
    ...
    def call(self, inputs):
        n_n = jnp.array(self.mobile_interior_vertices).shape[1]//2
        distancesx = jax.nn.softmax(jnp.array(self.mobile_interior_vertices)[0,:n_n])
        distancesy = jax.nn.softmax(jnp.array(self.mobile_interior_vertices)[0,n_n:])
        nodesx     = jnp.cumsum(distancesx)
        nodesy     = jnp.cumsum(distancesy)
        nodesx_with_fixed_point = jnp.insert(jnp.insert(nodesx, 0, 0.5),0,0).sort()
        nodesy_with_fixed_point = jnp.insert(jnp.insert(nodesy, 0, 0.5),0,0).sort()

        return jnp.concatenate([nodesx_with_fixed_point, nodesy_with_fixed_point])
